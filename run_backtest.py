"""
Hedge Fund OS — V6 Disciplined Alpha (FIXED).

Run: python3 run_v6_visual.py
...
def main():
    print("\n" + "=" * 65)
    print("  HEDGE FUND OS — V6: DISCIPLINED ALPHA")
    print("  Agent Team | Threshold Rebal | Early Warning | Conviction Size")
    print("=" * 65)
    ...
    from src.backtest.engine_v6 import BacktestEngineV6 as _E
    _test = _E(V6Config())
    ...
    # 1. Equity curve
    if not eq.empty:
        eq.to_csv(os.path.join(REPORTS, 'v6_equity_curve.csv'), index=False)
        print(f"  ✓ v6_equity_curve.csv")

    # 2. Trade log
    if not trades.empty:
        trades.to_csv(os.path.join(REPORTS, 'v6_trade_log.csv'), index=False)
        print(f"  ✓ v6_trade_log.csv")
    ...
    with open(os.path.join(REPORTS, 'v6_metrics.txt'), 'w') as f:
        for k, v in metrics_dict.items():
            f.write(f"{k}: {v}\n")
    print(f"  ✓ v6_metrics.txt")

    # 4. Agent decisions
    with open(os.path.join(REPORTS, 'v6_agent_decisions.json'), 'w') as f:
        json.dump(engine.agents.decisions[-500:], f, indent=2, default=str)
    print(f"  ✓ v6_agent_decisions.json ({len(engine.agents.decisions)} decisions)")
    ...
    if not eq.empty:
        ...
        fig.suptitle('HEDGE FUND OS V6 — Disciplined Alpha', fontsize=18, fontweight='bold', y=0.98)
        dates = pd.to_datetime(eq['date'])
        ax1.plot(dates, eq['equity'], color='#3B82F6', linewidth=2, label='V6 Strategy')
        ...
        ax1.set_title(
            f"V6: {metrics.total_return_pct:+.0f}% ({metrics.cagr_pct:+.1f}% CAGR, Sharpe {metrics.sharpe_ratio:.2f}) | "
        ...
        plt.tight_layout()
        p = os.path.join(REPORTS, 'v6_equity_vs_spy.png')
        plt.savefig(p, dpi=150, bbox_inches='tight')
        plt.close()
        charts.append(p)
        print("  ✓ v6_equity_vs_spy.png")

    # Chart 2: Trade analysis
    if not trades.empty:
        fig, axes = plt.subplots(2, 2, figsize=(16, 10))
        fig.suptitle(f'V6 Trade Analysis — {metrics.total_trades} trades, {metrics.win_rate:.0f}% win rate, {metrics.avg_hold_days:.0f}d avg hold',
        ...
        plt.tight_layout()
        p = os.path.join(REPORTS, 'v6_trade_analysis.png')
        plt.savefig(p, dpi=150, bbox_inches='tight')
        plt.close()
        charts.append(p)
        print("  ✓ v6_trade_analysis.png")
    ...
    print("  V6 COMPLETE")
    print("=" * 65)
    print(f"  V6:  ${CAPITAL:,.0f} → ${metrics.final_equity:,.0f} ({metrics.cagr_pct:+.1f}% CAGR)")
